...
Under most circumstances you should use the appropriate function instead of RawWrite
		...
		for item in data:
			self.bus.write_byte_data(self.i2cAddress, command, item)
...
Under most circumstances you should use the appropriate function instead of RawRead
		"""
		try:
			i2cRecv = self.bus.read_i2c_block_data(self.i2cAddress, command, length)
		except KeyboardInterrupt:
			raise
...
TB.printFunction = TB.NoPrint
	...
	def Init(self, try_other_bus = False):
		"""
		Init([try_other_bus])

		Prepare the I2C driver for talking to the ThunderBorg

		If try_other_bus is True, this function will attempt to use the other bus if the ThunderBorg
		devices can not be found on the current busNumber
		...
		try:
			...
			i2cRecv = self.bus.read_i2c_block_data(
				self.i2cAddress,
				COMMAND_GET_ID,
				I2C_MAX_LEN
			)
